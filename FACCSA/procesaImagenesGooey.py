...
@Gooey(program_name="Extractor de Facturas PDF")  # Decorador Gooey
def main():
    """Función principal del programa con interfaz Gooey."""

    parser = GooeyParser(description="Extrae datos de facturas PDF y guarda en Excel")
    parser.add_argument("pdf_path", metavar="Archivo PDF", widget="FileChooser", help="Ruta al archivo PDF con las facturas")
    parser.add_argument("-o", "--output_excel", metavar="Archivo Excel de Salida", widget="FileSaver", help="Nombre del archivo Excel para guardar los resultados", default="resultados.xlsx")

    args = parser.parse_args()  # Obtenemos los argumentos

    pdf_path = args.pdf_path
    nombre_excel = args.output_excel
    ...
    if not os.path.exists(pdf_path):
        ...
        procesar_pdf(pdf_path, nombre_excel)
