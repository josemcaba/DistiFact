...
ix, iy = -1, -1
fx, fy = -1, -1
rectangles = []  # Lista para almacenar las coordenadas de los rectángulos

# Función para manejar los eventos del ratón
def draw_rectangle(event, x, y, flags, param):
    global ix, iy, fx, fy, drawing, img, rectangles
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        drawing = False
        fx, fy = x, y
        cv2.rectangle(img, (ix, iy), (fx, fy), (0, 255, 0), 5)  # Grosor del borde aumentado
        cv2.imshow("Imagen", img)
        print(f"Rectángulo dibujado desde: ({ix}, {iy}) hasta ({fx}, {fy})")

        # Guardar las coordenadas del rectángulo en la lista
        rectangles.append({"x1": ix, "y1": iy, "x2": fx, "y2": fy})
